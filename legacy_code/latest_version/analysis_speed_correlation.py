...
import Bio, get_pdb_fold, get_tAI, csands_folds_reader, os, sys, random, itertools, re, scipy
...
org_keys = speed_gap.keys()
cov = np.ones((len(org_keys),len(org_keys)))
for i,j in itertools.combinations(xrange(len(org_keys)),2):
	org_1 = org_keys[i]
	org_2 = org_keys[j]

	a = get_tAI.get_tAI(org_1)
	b = get_tAI.get_tAI(org_2)
	x = []
	y = []
	for codon in a.keys():
		if a[codon] and b[codon]:
			x.append(a[codon])
			y.append(b[codon])
...
fig = plt.figure()
Y = sch.linkage(cov, method='centroid')
Z1 = sch.dendrogram(Y, orientation='right',labels=org_keys)
plt.show()

		
#		plt.figure()
#		plt.scatter(x,y)
#		plt.title('Codon speeds: '+org_1+' vs '+org_2)
#		slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(x, y)
#		plt.figtext(0.6,0.8,'R^2 = '+str(r_value**2))
#		plt.savefig('plots/'+org_1+'_'+org_2+'.png')
